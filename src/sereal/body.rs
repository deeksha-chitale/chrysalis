...
use crate::shared_model::{PerlValue, ValueRef};
...
pub fn read_value(cursor: &mut Cursor, seen: &mut SerealSeenTable, depth: usize) -> Result<ValueRef, SerealError> {
    ...
    let value = match tag {
        ...
        0x2C => {
            let class_tag_pos = cursor.pos();  // position of the class-name tag, about to be read
            let class_val = read_value(cursor, seen, depth + 1)?;
            let class = perl_value_to_string(&class_val)?;
            seen.register_class(class_tag_pos + 1, class.clone());  // +1 per the universal offset rule
            ...
            *inner.borrow_mut() = PerlValue::Blessed(PerlValue::wrap(unblessed), class);
            ...
            if track {
                seen.register(content_offset, inner.clone());
            }
            inner
        }
